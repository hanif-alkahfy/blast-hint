...
def draw_hint_block(block, top_x, top_y):
    for y in range(len(block)):
        for x in range(len(block[0])):
            if block[y][x] == 1:
                rect = pygame.Rect(
                    grid_offset_x + (top_x + x) * cell_size,
                    margin + (top_y + y) * cell_size,
                    cell_size, cell_size
                )
                pygame.draw.rect(screen, (0, 255, 0), rect, 2)  # outline kuning
...
def count_valid_placements(grid, blocks):
    count = 0
    for b in blocks:
        trimmed = trim_block(b["block"])
        for y in range(grid_size - len(trimmed) + 1):
            for x in range(grid_size - len(trimmed[0]) + 1):
                if can_place_block(grid, trimmed, x, y):
                    count += 1
    return count

def get_best_move(grid, blocks):
    best_score = -1
    best_move = None
        ...
        for y in range(grid_size - len(trimmed) + 1):
            for x in range(grid_size - len(trimmed[0]) + 1):
                if can_place_block(grid, trimmed, x, y):
                    ...
                    place_block(temp_grid, trimmed, x, y)
                    clear_full_lines(temp_grid)

                    # Strategi survival
                    empty_spaces = sum(row.count(0) for row in temp_grid)
                    sample_size = min(len(blocks), 3)
                    simulated_blocks = random.sample(list(blocks), sample_size)
                    valid_moves = count_valid_placements(temp_grid, simulated_blocks)

                    score = valid_moves * 10 + empty_spaces  # prioritas survivability
                    if score > best_score:
                        best_score = score
                        best_move = {"index": i, "x": x, "y": y}
    return best_move
...
while running:
    ...
    if hint_move:
        block = trim_block(available_blocks[hint_move["index"]]["block"])
        draw_hint_block(block, hint_move["x"], hint_move["y"])
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_h:
                hint_move = get_best_move(grid, available_blocks)
